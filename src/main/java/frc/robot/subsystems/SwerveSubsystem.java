...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public SwerveSubsystem(File directory) {
    ...
    try {
      swerveDrive = new SwerveParser(directory).createSwerveDrive(Constants.SwerveMotionConfig.MAX_SPEED);
    } catch (Exception e) {
  ...
  // Movimenta o robô com o joystick esquerdo, e mira o robo no ângulo no qual o
  // joystick está apontando
  ...
  // Movimenta o robô com o joystick esquerdo, e gira o robô na intensidade na
  // qual o joystick direito está para o lado
  ...
  public ChassisSpeeds getTargetSpeeds(double xInput, double yInput) {
    return swerveDrive.swerveController.getTargetSpeeds(xInput, yInput, 0, 0, 0, SwerveMotionConfig.MAX_SPEED);
  }
  ...
  public ChassisSpeeds discretize(ChassisSpeeds speeds) {
    var desiredDeltaPose = new Pose2d(
        speeds.vxMetersPerSecond * SwerveMotionConfig.DT,
        speeds.vyMetersPerSecond * SwerveMotionConfig.DT,
        new Rotation2d(speeds.omegaRadiansPerSecond * SwerveMotionConfig.DT * SwerveMotionConfig.CONSTANT_ROTATION));

    var twist = new Pose2d().log(desiredDeltaPose);

    return new ChassisSpeeds((twist.dx / SwerveMotionConfig.DT), (twist.dy / SwerveMotionConfig.DT),
